...
const decodeSignature = (
  ...
  Bytes.slice(64, 65, hex),
  Bytes.slice(0, 32, hex),
  Bytes.slice(32, 64, hex),
];

const makeSign = (addToV) =>
  (hash, privateKey) => {
...
const signTransaction = (tx, privateKey) => {
  ...
  const signature = makeSign(Nat.toNumber(tx.chainId || "0x1") * 2 + 35)(
    keccak256(signingData),
    privateKey,
  );
  const rawTransaction = rlp.decode(signingData).slice(0, 6).concat(
