...
export default function Api(provider) {
  const send = (method) =>
    (...params) =>
  ...
  const addTransactionDefaults = (tx) =>
    ...
    Promise.all([
      ...
      .then(([chainId, gasPrice, nonce, value, data]) => {
        return Map.merge(tx)(
          { chainId: Nat.fromNumber(chainId), gasPrice, nonce, value, data },
        );
      })
      .then((tx) => {
      ...
      if (method && method.name) {
        ...
        if (!method.constant) {
          ...
          contract[method.name + "_pay"] = (value) =>
            (...params) => call("receipt", value)(...params);
          contract[method.name + "_pay_txHash"] = (value) =>
            (...params) => call("txHash", value)(...params);
