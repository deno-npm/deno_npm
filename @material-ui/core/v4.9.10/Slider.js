import _slicedToArray from "https://cdn.skypack.dev/@babel/runtime/helpers/esm/slicedToArray";
...
import { fade, lighten, darken } from "./styles/colorManipulator.js";
...
import ValueLabel from "./Slider/ValueLabel.js";
...
function findClosest(values, currentValue) {
  var _values$reduce = values.reduce(function (acc, value, index) {
      ...
      if (
        acc === null || distance < acc.distance || distance === acc.distance
      ) {
        return {
          distance: distance,
          index: index,
...
function getDecimalPrecision(num) {
  ...
  if (Math.abs(num) < 1) {
    ...
    return (matissaDecimalPart ? matissaDecimalPart.length : 0) +
      parseInt(parts[1], 10);
  }
...
function focusThumb(_ref2) {
  ...
  if (
    ...
    sliderRef.current.querySelector('[data-index="'.concat(activeIndex, '"]'))
      .focus();
  }
...
export var styles = function styles(theme) {
  return {
    ...
    thumb: {
      ...
      "&$focusVisible,&:hover": {
        boxShadow: "0px 0px 0px 8px ".concat(
          fade(theme.palette.primary.main, 0.16),
        ),
        "@media (hover: none)": {
          boxShadow: "none",
        },
    ...
    thumbColorSecondary: {
      "&$focusVisible,&:hover": {
        boxShadow: "0px 0px 0px 8px ".concat(
          fade(theme.palette.secondary.main, 0.16),
        ),
      },
      "&$active": {
        boxShadow: "0px 0px 0px 14px ".concat(
...
var Slider = React.forwardRef(function Slider(props, ref) {
  var ariaLabel = props["aria-label"],
    ...
    ThumbComponent = _props$ThumbComponent === void 0
      ? "span"
    ...
    ValueLabelComponent = _props$ValueLabelComp === void 0 ? ValueLabel
    : _props$ValueLabelComp,
    _props$valueLabelDisp = props.valueLabelDisplay,
    valueLabelDisplay = _props$valueLabelDisp === void 0 ? "off"
    : _props$valueLabelDisp,
    _props$valueLabelForm = props.valueLabelFormat,
    valueLabelFormat = _props$valueLabelForm === void 0 ? Identity
  ...
  var values = range
    ? _toConsumableArray(valueDerived).sort(asc)
    : [valueDerived];
    ...
    switch (event.key) {
      ...
      case "ArrowUp":
        if (step) {
          ...
          newValue = marksValues[marksIndex + 1] ||
  ...
  var trackLeap = valueToPercent(values[values.length - 1], min, max) -
    trackOffset;
  ...
  return /*#__PURE__*/ React.createElement(
    ...
    _extends({
      ...
      className: clsx(
        ...
        marks.length > 0 && marks.some(function (mark) {
          return mark.label;
        }) && classes.marked,
    ...
    marks.map(function (mark, index) {
      ...
      if (track === false) {
        ...
        markActive = track === "normal" && (range
              ? mark.value >= values[0] &&
                mark.value <= values[values.length - 1]
              : mark.value <= values[0]) || track === "inverted" && (range
                ? mark.value <= values[0] ||
                  mark.value >= values[values.length - 1]
                : mark.value >= values[0]);
      }
