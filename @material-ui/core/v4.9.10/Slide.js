...
import { reflow, getTransitionProps } from "./transitions/utils.js"; // Translate the node so he can't be seen on the screen.
...
function getTranslateValue(direction, node) {
  ...
  if (direction === "left") {
    return "translateX(".concat(window.innerWidth, "px) translateX(-").concat(
      rect.left - offsetX,
      "px)",
    );
  }
  ...
  if (direction === "up") {
    return "translateY(".concat(window.innerHeight, "px) translateY(-").concat(
      rect.top - offsetY,
      "px)",
    );
  } // direction === 'down'
