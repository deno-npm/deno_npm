...
var styles = function styles(theme) {
  ...
  return theme.breakpoints.keys.reduce(function (acc, key) {
    acc["only".concat(capitalize(key))] = _defineProperty(
      {},
      theme.breakpoints.only(key),
      hidden,
    );
    acc["".concat(key, "Up")] = _defineProperty(
      {},
      theme.breakpoints.up(key),
      hidden,
    );
    acc["".concat(key, "Down")] = _defineProperty(
      {},
...
function HiddenCss(props) {
  var children = props.children,
    ...
    other = _objectWithoutProperties(
      props,
      ["children", "classes", "className", "only"],
    );
  ...
  var unknownProps = Object.keys(other).filter(function (propName) {
    var isUndeclaredBreakpoint = !theme.breakpoints.keys.some(
      function (breakpoint) {
        return "".concat(breakpoint, "Up") === propName ||
  ...
  if (unknownProps.length > 0) {
    console.error(
      'Material-UI: unsupported props received by `<Hidden implementation="css" />`: '
        .concat(
          unknownProps.join(", "),
