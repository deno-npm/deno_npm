...
export default {
  ...
  create: function create() {
    ...
    var _options$duration = options.duration,
      durationOption = _options$duration === void 0
        ? duration.standard
        : _options$duration,
      _options$easing = options.easing,
      easingOption = _options$easing === void 0 ? easing.easeInOut
      : _options$easing,
    ...
    if (!isNumber(durationOption) && !isString(durationOption)) {
      console.error(
        'Material-UI: argument "duration" must be a number or a string but found '
          .concat(durationOption, "."),
      );
    }
    ...
    return (Array.isArray(props) ? props : [props]).map(
      function (animatedProp) {
        return "".concat(animatedProp, " ").concat(
